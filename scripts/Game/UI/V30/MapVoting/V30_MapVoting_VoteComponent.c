class V30_MapVoting_VoteComponent : ScriptedWidgetComponent {
	...
	protected string m_VoteCountWidgetName;

	[Attribute("", desc: "Name of widget for check mark.", uiwidget: UIWidgets.EditBox)];
	protected string m_CheckWidgetName;
	...
	override event void HandlerAttached(Widget w) {
		...
		m_VoteCountWidget = TextWidget.Cast(m_RootWidget.FindAnyWidget(m_VoteCountWidgetName));
		m_CheckWidget = m_RootWidget.FindAnyWidget(m_CheckWidgetName);
		if (m_CheckWidget) m_CheckWidget.SetVisible(false);
		m_ImageWidget = ImageWidget.Cast(m_RootWidget.FindAnyWidget(m_ImageWidgetName));
		m_TitleWidget = TextWidget.Cast(m_RootWidget.FindAnyWidget(m_TitleWidgetName));
		m_SubTitleWidget = TextWidget.Cast(m_RootWidget.FindAnyWidget(m_SubTitleWidgetName));
